...
const uppercaseFirstLetter = (word) => word.replace(/\w/, (firstLetter) => firstLetter.toUpperCase());

const formattedExportString = (namedExport) =>
  `export { default as ${uppercaseFirstLetter(namedExport)} } from './${uppercaseFirstLetter(namedExport)}';`;
...
const getDirectoryNames = async () => {
  ...
  return directories
    ...
    .filter(
      (directory) => !(directory.match(/index.ts/i) || directory.match(/snippets.ts/i) || directory.match(/.DS_Store/i))
    );
};
...
module.exports = {
  prompt: async ({ prompter }) => {
    ...
    return prompter
      ...
      .then(({ name }) => {
        !data?.includes(formattedExportString(name)) && data.push(formattedExportString(name));
